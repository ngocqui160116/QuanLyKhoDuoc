...
namespace Phoenix.Server.Services.MainServices.Media
{
    public class ImageService
    {
        ...
        {
            var image = await _dataContext.Medicine_Images.FindAsync(id);
            if (image == null || image.Deleted) return null;
            return image.MapTo<ImageModel>();
        }
        public IPagedList<Medicine_Image> SearchAllImage(int page = 0, int pageSize = 20,
            string title = null)
        {
            var query = _dataContext.Medicine_Images.AsNoTracking().AsQueryable();
            if (!string.IsNullOrEmpty(title))
            {
                query = query.Where(x => x.FileName.Contains(title));
            }
            query = query.OrderByDescending(x => x.CreatedAt);
            return new PagedList<Medicine_Image>(query, page, pageSize);
        }
        public Medicine_Image WriteImageFile(string host, string fileName, byte[] content, string path = "")
        {
            ...
            _dataContext.Medicine_Images.Add(img);
        ...
        public void DeleteImageFileInDisk(int id, string path="")
        {
            ...
            //var relPath = "/" + Path.Combine("Upload", pathTime, path.Replace('\\', '/'));
            //var fullPath = CommonHelper.MapPath(relPath.Replace('\\', '/'));
            var img = _dataContext.Medicine_Images.Find(id);
            if(img != null)
            {
                ...
                _dataContext.Medicine_Images.Remove(img);
                _dataContext.SaveChanges();
            }    
        ...
        public Medicine_Image ReplaceOldImage(int? imageId, string host, string fileName, byte[] content, string path = "")
        {
            host = string.IsNullOrEmpty(host) ? HostAddress : host;
            string extension = string.Empty;
            if (fileName.Length > 4)
            ...
            if (imageId.HasValue)
            {
                ...
                //string base64String = Convert.ToBase64String(content);
                File.WriteAllBytes(Path.Combine(fullPath, fileName), content);
                var oldImage = _dataContext.Medicine_Images.Find(imageId);
        ...
        public Medicine_Image GetImageById(int id) => _dataContext.Medicine_Images.Find(id);
        public bool DeleteImage(Medicine_Image entity)
        {
            if (entity == null)
            ...
            try
            {
                _dataContext.Medicine_Images.Remove(entity);
                _dataContext.SaveChanges();
                result = true;
            }
            catch (Exception e)
